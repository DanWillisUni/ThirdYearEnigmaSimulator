    ...
    public class BasicService
    {
      ...
      },
      {
        ...
        public void root() {
            ...
            emRotors.Add(new RotorModel(allRotors[0]));
            emRotors.Add(new RotorModel(allRotors[1]));
            emRotors.Add(new RotorModel(allRotors[2]));
            EnigmaModel em = new EnigmaModel(emRotors, new RotorModel(allReflectors[1]), new Dictionary<int, int>());
...
";//first 4 paragraphs in alice in wonderland
            ...
            List<BreakerResult> rotorResults = getRotorResults(ciphertext,_resolver("IOC"));
                                ...
                                for (int l = 0; l <= 25; l++)
                                {
                                    for (int m = 0; m <= 25; m++)
                                    {
                                        for (int r = 0; r <= 25; r++)
                                        {
                                            ...
                                            EnigmaModel em = new EnigmaModel(rotors, new RotorModel(allReflectors[0]), new Dictionary<int, int>());
